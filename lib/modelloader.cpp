...
string loadMaterialTextures(aiMaterial *mat, aiTextureType type, string directory, unordered_map<string, unsigned> &texturesLoaded, vector<unsigned> &textureVector) {
	stringstream errStr;
	for(unsigned int i = 0; i < mat->GetTextureCount(type); i++) {
		...
		if(texturesLoaded.count(filename) == 0) {
			...
			textureVector.push_back(texturesLoaded[filename]);
		}
	}
	return errStr.str();
}
...
string createMesh(const aiScene* scene, glmodel_dl *model, string directory, int vPosAttr, int vNorAttr, int vTexAttrib, int vTanAttrib, int vBitAttrib, int vBoneIDAttrib, int vWeightAttrib) {
	...
	while(!nodeTreeStack.empty()) {
		...
		for(unsigned int i = 0; i < node->mNumMeshes; i++) {
			...
			aiColor3D specolor(0.0f, 0.0f, 0.0f);
			material->Get(AI_MATKEY_COLOR_EMISSIVE, specolor);
			m.emissionColor = vec3(specolor.r, specolor.g, specolor.b);
			errStr<<loadMaterialTextures(material, aiTextureType_DIFFUSE, directory, loadedTexture, m.diffuseTextures);
			errStr<<loadMaterialTextures(material, aiTextureType_SPECULAR, directory, loadedTexture, m.specularTextures);
